...
void defaultSettings() {
  ...
  appButtonBackgroundColorGlobal = primary;
  defaultRadius = 8;
...
int getRemainingPlanDays() {
  if (appStore.planEndDate.isNotEmpty) {
    ...
    final DateTime endAt =
        DateFormat(DATE_FORMAT_7).parse(appStore.planEndDate);
...
void launchMapFromLatLng(
    {required double latitude, required double longitude}) {
  final String query = "${latitude.toString()},${longitude.toString()}";
  commonLaunchUrl(GOOGLE_MAP_PREFIX + Uri.encodeFull(query),
      launchMode: LaunchMode.externalApplication);
}
...
void forceUpdate(
  ...
  showInDialog(
    ...
    barrierDismissible:
        currentAppVersionCode >= getIntAsync(PROVIDER_APP_MINIMUM_VERSION),
    builder: (_) {
      return PopScope(
        canPop: false,
        child: NewUpdateDialog(
          canClose: getBoolAsync(UPDATE_NOTIFY)
              ? currentAppVersionCode >=
                  getIntAsync(PROVIDER_APP_MINIMUM_VERSION)
              : true,
        ),
      );
    },
...
Future<void> showForceUpdateDialog(BuildContext context) async {
  if (getBoolAsync(UPDATE_NOTIFY, defaultValue: true)) {
    getPackageInfo().then((value) {
      if ((isAndroid || isIOS) &&
          getIntAsync(PROVIDER_APP_LATEST_VERSION) >
...
String convertToHourMinute(String timeStr) {
  ...
  final int hours =
      int.parse(parts[0]) % 24; // Ensure hours are within 24 hours
          ...
          child: SingleChildScrollView(
            ...
            child: Row(
              children: List.generate(
                languageList().length,
                (index) {
                  final LanguageDataModel languageData = languageList()[index];
                  return ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      side: BorderSide(color: context.icon),
