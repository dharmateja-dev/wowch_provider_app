...
Future<CategoryResponse> getSubCategoryList(
    {required int catId, String? languageCode}) async {
...
Future<DashboardResponse> providerDashboard({
  bool forceSyncAppConfigurations = false,
}) async {
...
Future<List<BookingStatusResponse>> bookingStatus({
  required List<BookingStatusResponse> list,
}) async {
  Iterable res = await handleResponse(
...
Future<num> getUserWalletBalance() async {
  try {
    var res = WalletResponse.fromJson(
      await handleResponse(
        await buildHttpResponse(
          'user-wallet-balance',
          method: HttpMethodType.GET,
...
Future<SelectZoneModelResponse> selectedZones({int? providerId}) async {
  return SelectZoneModelResponse.fromJson(
    await handleResponse(
      await buildHttpResponse(
        'provider-zones?provider_id=$providerId',
        method: HttpMethodType.GET,
      ),
    ),
  );
...
Future<List<RequestListModel>> getRequestList(
  int page,
  int id,
  List<RequestListModel> list,
  Function(bool)? lastPageCallback,
) async {
  var res = RequestListResponse.fromJson(
    await handleResponse(
      await buildHttpResponse(
        'provider-payout-list?provider_id=$id&page=$page',
        method: HttpMethodType.GET,
      ),
    ),
  );
...
Future<UserTypeResponse> getCommissionType({
  String type = USER_TYPE_PROVIDER,
  int? providerId,
  bool? isDelete,
}) async {
  ...
  if (imageFile != null) {
    multiPartRequest.files.add(
      await MultipartFile.fromPath(
        AddonServiceKey.serviceAddonImage,
        imageFile.path,
      ),
    );
  }
...
Future<void> editShop(Map<String, dynamic> data, List<File> images,
  ...
  if (images.isNotEmpty) {
    ...
    print('No images to upload for edit');
  }
  multiPartRequest.headers.addAll(buildHeaderTokens());
...
Future<void> addEditShopMultiPart({
  ...
  String endpoint = shopId > 0 ? 'shop-update/$shopId' : 'shop-create';
  ...
  multiPartRequest.headers.addAll(buildHeaderTokens());
  await sendMultiPartRequest(multiPartRequest, onSuccess: (temp) async {
    BaseResponseModel res = BaseResponseModel.fromJson(jsonDecode(temp));
    toast(res.message);
  }, onError: (error) {
    toast(error.toString(), print: true);
  }).catchError((e) {
...
//region shop document

Future<List<ProviderDocuments>> getShopDoc({
  int page = 1,
  List<ProviderDocuments> documents = const [],
  Function(bool)? lastPageCallback,
  var perPage = PER_PAGE_ITEM,
}) async {
  ProviderDocumentListResponse response = ProviderDocumentListResponse.fromJson(
    await handleResponse(
      await buildHttpResponse('shop-document-list', method: HttpMethodType.GET),
    ),
  );
