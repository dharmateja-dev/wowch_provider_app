...
class EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> userDetailAPI() async {
    await getUserDetail(appStore.userId).then((value) async {
      final String tempLanguages = value.data!.knownLanguages.validate();
      ...
      if (tempLanguages.isNotEmpty && tempLanguages.isJson()) {
        final Iterable it = jsonDecode(tempLanguages);
      ...
      final String tempSkills = value.data!.skills.validate();

      if (tempSkills.isNotEmpty && tempSkills.isJson()) {
        final Iterable it = jsonDecode(tempSkills);
        skills.addAll(it.map((e) => e.toString()).toList());
      }
      descriptionCont.text =
          value.data!.whyChooseMeObj.aboutDescription.validate();
      addressCont.text = value.data!.address.validate();
      lNameCont.text = value.data!.lastName.validate();
      fNameCont.text = value.data!.firstName.validate();

      // Parse country calling code from stored contact and set picker/number
      final String raw = value.data!.contactNumber.validate().trim();
      if (raw.startsWith('+')) {
        final RegExp re = RegExp(r'^\+(\d{1,3})\s*(.*)$');
        final Match? m = re.firstMatch(raw);
        if (m != null) {
          final String callingCode = m.group(1) ?? '';
          final String local =
              (m.group(2) ?? '').replaceAll(RegExp(r'[^\d]'), '');

          selectedCountryPicker = Country(
            phoneCode: callingCode.replaceAll('+', ''),
            countryCode: "",
            e164Sc: 0,
            geographic: true,
            level: 0,
      ...
      userNameCont.text = value.data!.username.validate();
  ...
  Future<void> update() async {
    ...
    multiPartRequest.fields[UserKeys.contactNumber] = mobileCont.text.isEmpty
        ? ''
        : '+${mobileCont.text.trim().formatPhoneNumber(selectedCountryPicker.phoneCode)}';
