...
//-------------------------------- Variables -------------------------------//
  ...
  void init() async {
    ...
    if (isUpdate) {
      ...
      addonNameCont.text = widget
              .addonServiceData?.translations?[DEFAULT_LANGUAGE]?.name
              .validate()
              .trim() ??
          "";
      addonPriceCont.text =
  ...
  Future<void> reloadDataForLanguage() async {
    try {
      ...
      if (appStore.selectedServiceData.id != null) {
        ...
        final idx =
            list.indexWhere((s) => s.id == appStore.selectedServiceData.id);
        if (idx != -1) appStore.setSelectedServiceData(list[idx]);
      }
    } catch (e) {
      log(e.toString());
    } finally {
