...
class _AddServicesState extends State<AddServices> {
  ...
  List<VisitTypeData> visitTypeData = [
    ...
    if (appConfigurationStore.digitalServiceStatus)
      VisitTypeData(
          isEnabled: false, title: languages.shop, key: VISIT_OPTION_SHOP),
  ];
  ...
  void init() async {
    ...
    if (isUpdate) {
      ...
      imageFiles = widget.data!.serviceDetail!.attchments
          .validate()
          .map((e) => File(e.url.toString()))
          .toList();
      serviceNameCont.text = widget
              .data?.serviceDetail?.translations?[DEFAULT_LANGUAGE]?.name
              .validate() ??
          "";
      priceCont.text = widget.data!.serviceDetail!.price.toString().validate();
      discountCont.text =
          widget.data!.serviceDetail!.discount.toString().validate();
      descriptionCont.text = widget
              .data?.serviceDetail?.translations?[DEFAULT_LANGUAGE]?.description
              .validate() ??
          "";
      categoryId = widget.data!.serviceDetail!.categoryId.validate();
      subCategoryId = widget.data!.serviceDetail!.subCategoryId.validate();
      isFeature =
          widget.data!.serviceDetail!.isFeatured.validate() == 1 ? true : false;
      serviceType = widget.data!.serviceDetail!.type.validate();
      serviceStatus = widget.data!.serviceDetail!.status.validate() == 1
          ? ACTIVE
          : INACTIVE;
      ...
      serviceZoneList =
          widget.data?.zones?.map((e) => e.id.validate()).toList() ?? [];

      currentTime = TimeOfDay(
          hour: widget.data!.serviceDetail!.duration
              .validate()
              .splitBefore(':')
              .toInt(),
          minute: widget.data!.serviceDetail!.duration
              .validate()
              .splitAfter(':')
              .toInt());
      ...
      isTimeSlotAvailable =
          widget.data!.serviceDetail!.isSlot.validate() == 1 ? true : false;
      isAdvancePayment = widget.data!.serviceDetail!.isAdvancePayment;
      if (widget.data!.serviceDetail!.advancePaymentAmount != null) {
        prePayAmountController.text = widget
            .data!.serviceDetail!.advancePaymentAmount
            .validate()
            .toString();
      }
      ...
      timeSlotStore.initializeSlots(
          value: widget.data!.serviceDetail!.providerSlotData.validate());

      selectedVisitType = visitTypeData.firstWhere(
          (element) =>
              element.key == widget.data!.serviceDetail!.visitType.validate(),
    ...
    await getShops(
        serviceId: isUpdate ? widget.data!.serviceDetail!.id.validate() : 0);
  }
  ...
  Future<void> reloadDataForLanguage() async {
    // Only trigger rebuilds so that widgets depending on languageCode refetch themselves
    setState(() {
...
//region Add Service
  Future<void> checkValidation(
    ...
    if ((!isUpdate && serviceZoneList.validate().isEmpty) ||
        (isUpdate && serviceZoneList.validate().isEmpty)) {
        ...
        try {
          if (isUpdate) {
            await getServiceDetail(
                {'service_id': widget.data!.serviceDetail!.id.validate()});
          }
        } catch (e) {}
      }
    }
...
//region service request
  Map<String, dynamic> _buildServiceRequest() {
    ...
    if (isUpdate) {
      req.putIfAbsent(
          AddServiceKey.id, () => widget.data!.serviceDetail!.id.validate());
      req.putIfAbsent(AddServiceKey.providerZoneId,
          () => widget.data!.zones?.map((e) => e.id.validate()).toList());
    }
    ...
    if (selectedVisitType!.key == VISIT_OPTION_SHOP) {
      if (selectedShops.isEmpty) {
        ...
        final shopIds = selectedShops
            .where((element) => element.isSelected)
            .map((e) => e.id.validate())
            .toList();
        req.putIfAbsent(AddServiceKey.shopIds, () => shopIds);
      }
    }
          ...
          children: [
            ...
            if (selectedVisitType?.key == VISIT_OPTION_SHOP)
              ServiceShopComponent(
                selectedList: widget.data?.shop
                    .map((shop) => shop.id.validate())
                    .toList(),
                onSelectedList: (val) {
