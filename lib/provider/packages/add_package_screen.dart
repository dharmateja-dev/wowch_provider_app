...
class _AddPackageScreenState extends State<AddPackageScreen> {
  ...
  void init() async {
    ...
    appStore.setSelectedLanguage(localeLanguageList
        .firstWhere((element) => element.languageCode == DEFAULT_LANGUAGE));
    if (isUpdate) {
      ...
      selectedSubCategoryId =
          widget.data!.subCategoryId ?? selectedSubCategoryId;
  ...
  Future<void> reloadDataForLanguage() async {
    try {
      // Refresh selected service names in the current language
      final response = await getServicesList(
        1,
      ...
      if (appStore.selectedServiceList.isNotEmpty) {
        final list = response.data.validate();
        for (int i = 0; i < appStore.selectedServiceList.length; i++) {
          final sel = appStore.selectedServiceList[i];
          final idx = list.indexWhere((s) => s.id == sel.id);
          if (idx != -1) {
            // replace stored item with localized one
            appStore.selectedServiceList[i] = list[idx];
          }
        }
      }
    } catch (e) {
